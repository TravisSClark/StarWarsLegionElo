...
def get_tournament_players(groups):
    ...
    for group in groups:
        for player in group["groupPlayerResults"]:
            if (not int(player["player"]["user"]["id"]) in player_id_elo_dict) and (not player in new_player_list):
...
def get_game_results(player_elo_dict, players):
    for player in players:
        ...
        player_elo_dict[player_id]["games"] += player["wonMatchesAmount"] + player["lostMatchesAmount"]
